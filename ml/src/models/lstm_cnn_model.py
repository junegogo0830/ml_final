"""
CNN + LSTM 기업 파산 예측 모델
CNN이 로컬 패턴 추출 → LSTM이 시계열 흐름 학습
실행: python cnn_lstm_model.py
...
class CNNLSTMModel(nn.Module):
    """
    CNN + LSTM 구조:
      CNN:  로컬 시계열 패턴 추출 (급격한 변화, 단기 추세)
      LSTM: 전체 시계열 흐름 학습 (장기 악화 궤적)
    """
    def __init__(self, input_size, cnn_filters=64, kernel_size=2,
                 hidden_size=64, num_layers=2, dropout=0.3):
        super().__init__()

        # CNN 블록: 시계열에서 로컬 패턴 추출
        self.cnn = nn.Sequential(
            # 1D Convolution (time 축 방향)
    ...
    def forward(self, x):
        ...
        # CNN: (batch, features, seq_len) → (batch, filters, seq_len)
        cnn_in  = x.permute(0, 2, 1)
        cnn_out = self.cnn(cnn_in)

        # LSTM: (batch, seq_len, filters)
...
def train_model(train_df, feature_cols):
    ...
    model = CNNLSTMModel(
        len(feature_cols), CNN_FILTERS, KERNEL_SIZE,
    ...
    print(f"\nCNN+LSTM 학습 시작 | 파라미터: {sum(p.numel() for p in model.parameters()):,}개")
...
def evaluate(model, test_df, feature_cols):
    ...
    pd.DataFrame({'true_label': labels, 'pred_prob': probs, 'pred_label': preds})\
      .to_csv("data/predictions/cnn_lstm_predictions.csv", index=False)
    print("예측 결과 저장: cnn_lstm_predictions.csv")
