"""
TFT (Temporal Fusion Transformer) - 순수 PyTorch 구현
pytorch-forecasting 없이 torch만 사용
실행: python tft_model.py
"""
...
class VariableSelectionNetwork(nn.Module):
    def __init__(self, num_vars, hidden_size, dropout=0.1):
        ...
        self.select_grn = GatedResidualNetwork(num_vars, hidden_size, num_vars, dropout)

    def forward(self, x):
        B, T, V = x.shape
        embeds = torch.stack([self.var_grns[i](x[:,:,i:i+1]) for i in range(V)], dim=2)
        flat   = x.reshape(B * T, V)
        w      = torch.softmax(self.select_grn(flat).reshape(B, T, V, 1), dim=2)
        return (embeds * w).sum(dim=2), w.squeeze(-1)


class TFT(nn.Module):
    def __init__(self, num_features, hidden_size=32, num_heads=2, dropout=0.1):
        super().__init__()
        self.vsn      = VariableSelectionNetwork(num_features, hidden_size, dropout)
        self.lstm     = nn.LSTM(hidden_size, hidden_size, batch_first=True)
        self.lstm_grn = GatedResidualNetwork(hidden_size, hidden_size, hidden_size, dropout)
    ...
    def forward(self, x):
        ...
        return self.head(ffn_out[:, -1, :]).squeeze(-1), var_w
...
def train_model(train_df, feature_cols):
    ...
    model     = TFT(len(feature_cols), HIDDEN_SIZE, N_HEADS, DROPOUT).to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=1e-5)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=5, factor=0.5)
    criterion = nn.BCELoss()

    print(f"\nTFT 학습 시작 | 파라미터: {sum(p.numel() for p in model.parameters()):,}개")
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for X, y in val_loader:
                X, y = X.to(DEVICE), y.to(DEVICE)
                prob, _ = model(X)
                val_loss += criterion(prob, y).item()
...
def evaluate(model, test_df, feature_cols):
    ...
    weights_mean = np.concatenate(all_weights).mean(axis=0)
    print("\n=== 변수 중요도 (상위 10개) ===")
    for feat, imp in sorted(zip(feature_cols, weights_mean), key=lambda x: x[1], reverse=True)[:10]:
        print(f"  {feat:<25} {imp:.4f} {'█' * int(imp * 50)}")

    pd.DataFrame({'true_label': labels, 'pred_prob': probs, 'pred_label': preds})\
      .to_csv("data/predictions/tft_predictions.csv", index=False)
    print(f"\n예측 결과 저장: tft_predictions.csv")
